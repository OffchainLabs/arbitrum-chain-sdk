...
// TIMESTAMP: 2026-07-17T16:18:23.852Z
//
// ---
...
export type NodeConfig = {
  ...
  'blocks-reexecutor'?: {
    ...
    'blocks'?: string;
    /** if set, blocks-reexecutor not only re-executes blocks but it also commits their state to triedb */
    'commit-state-to-disk'?: boolean;
    ...
    'mode'?: string;
    /** number of threads to parallelize blocks re-execution (default 14) */
    'room'?: number;
    /** memory allowance (MB) to use for caching trie nodes in memory */
    'trie-clean-limit'?: number;
    /** if set, validate the sum of multi-gas dimensions match the single-gas */
    'validate-multigas'?: boolean;
  };
  'chain'?: {
    'dev-wallet'?: {
  ...
  'conf'?: {
    ...
    's3'?: {
      /** S3 access key for fetching the node configuration file from S3 */
      'access-key'?: string;
      /** S3 bucket containing the node configuration file */
      'bucket'?: string;
      /** S3 object key of the node configuration file (JSON format) */
      'object-key'?: string;
      /** S3 region of the bucket containing the node configuration file */
      'region'?: string;
      /** S3 secret key for fetching the node configuration file from S3 (triggers S3 config loading) */
      'secret-key'?: string;
    };
  ...
  'execution'?: {
    ...
    'caching'?: {
      ...
      /** enable recording of preimages */
      'enable-preimages'?: boolean;
      /** maximum number of blocks rolled back to recover chain head (0 = use geth default limit) (default 2419200) */
      'head-rewind-blocks-limit'?: number;
      ...
      /** maximum number of diff layers to keep in pathdb (path state-scheme only) (default 128) */
      'pathdb-max-diff-layers'?: number;
      ...
      /** number of recent blocks to retain state history for (path state-scheme only) (default 18446744073709551615) */
      'state-history'?: number;
      /** scheme to use for state trie storage (hash, path) (default "hash") */
      'state-scheme'?: string;
      /** enable tracking of state size over time */
      'state-size-tracking'?: boolean;
      ...
      /** batch size in bytes used in the TrieDB Cap operation (0 = use geth default) */
      'trie-cap-batch-size'?: number;
      ...
      /** batch size in bytes used in the TrieDB Commit operation (0 = use geth default) */
      'trie-commit-batch-size'?: number;
      ...
      /** Execution will suggest that maintenance is run if the block processing time required to reach trie-time-limit is smaller or equal than trie-time-limit-before-flush-maintenance */
      'trie-time-limit-before-flush-maintenance'?: string;
    ...
    /** disable ArbOwner precompile calls outside on-chain execution (ethcall, gas estimation) */
    'disable-arbowner-ethcall'?: boolean;
    /** enable prefetching of blocks (default true) */
    'enable-prefetch-block'?: boolean;
    /** experimental: expose multi-dimensional gas in transaction receipts */
    'expose-multi-gas'?: boolean;
    'forwarder'?: {
      ...
      /** the Redis URL to recommend target via */
      'redis-url'?: string;
    ...
    /** orbit-chain compat: re-enables the pre-v3.7 behavior of treating ArbOS<=40 blocks with zero base fee as non-arbitrum, for blocks with unix timestamp strictly less than this value (0 disables; set to a timestamp past the last zero-basefee block on the chain) */
    'legacy-zero-base-fee-until'?: number;
    'parent-chain-reader'?: {
      'dangerous'?: {
    ...
    'rpc-server'?: {
      /** rpc is authenticated (default true) */
      authenticated?: boolean;
      /** enable execution node to serve over rpc */
      enable?: boolean;
      /** rpc is public */
      public?: boolean;
    };
    'rpc'?: {
      ...
      /** array of node configs to redirect block requests given as a json string. time duration should be supplied in number indicating nanoseconds (default "default") */
      'block-redirects-list'?: string;
      ...
      'gas-cap'?: number;
      /** export log index checkpoints to file */
      'log-export-checkpoints'?: string;
      /** maximum number of blocks from head where a log search index is maintained (default 9400000) */
      'log-history'?: number;
      /** no log search index is maintained */
      'log-no-history'?: boolean;
      ...
      'tx-fee-cap'?: number;
      /** default timeout for eth_sendRawTransactionSync (default 20s) */
      'tx-sync-default-timeout'?: string;
      /** maximum allowed timeout for eth_sendRawTransactionSync  (default 1m0s) */
      'tx-sync-max-timeout'?: string;
    };
    /** secondary transaction forwarding target URL */
    'secondary-forwarding-target'?: string[];
    'sequencer'?: {
      'dangerous'?: {
        /** DANGEROUS! disables nitro checks on sequencer for blob base fee */
        'disable-blob-base-fee-check'?: boolean;
      ...
      /** gas price setting to be used in calculating estimated surplus. Allowed values- CalldataPrice, BlobPrice and CalldataPrice7523 (default "BlobPrice") */
      'expected-surplus-gas-price-mode'?: string;
      ...
      /** timeout for reading new messages (default 10ms) */
      'read-from-tx-queue-timeout'?: string;
    ...
    'stylus-target'?: {
      /** if true, fall back to an alternative compiler when compilation of a Stylus program fails (default true) */
      'allow-fallback'?: boolean;
      ...
      'host'?: string;
      /** max number of Stylus frames simultaneously on the call stack (counts only Stylus frames; EVM frames between two Stylus frames do not decrement it); exceeding the limit rejects non-on-chain calls; 0 disables the limit */
      'max-stylus-call-depth'?: number;
      /** max open WASM pages per tx; exceeding the limit rejects non-on-chain calls and filters sequencer-committed txs (delayed inbox is exempt); 0 disables the limit (default 128) */
      'max-stylus-open-pages'?: number;
      /** initial native stack size in bytes for Wasmer coroutines used by Stylus execution (0 = default 1MB) */
      'native-stack-size'?: number;
    };
    'sync-monitor'?: {
      ...
      /** allowed message lag while still considered in sync (default 1s) */
      'msg-lag'?: string;
    ...
    'transaction-filtering'?: {
      'address-filter'?: {
        /** work queue size for address checker (default 8192) */
        'address-checker-queue-size'?: number;
        /** number of workers for address checker (default 4) */
        'address-checker-worker-count'?: number;
        /** LRU cache size for address lookup results (default 10000) */
        'cache-size'?: number;
        /** interval between polling S3 for hash list updates (default 5m0s) */
        'poll-interval'?: string;
        's3'?: {
          /** S3 access key */
          'access-key'?: string;
          /** S3 bucket name */
          'bucket'?: string;
          /** S3 multipart download part size in MB (default 32) */
          'chunk-size-mb'?: number;
          /** S3 multipart download concurrency (default 10) */
          'concurrency'?: number;
          /** custom S3 endpoint URL (for MinIO, localstack, or other S3-compatible services) */
          'endpoint'?: string;
          /** maximum allowed S3 object size in MB; if the object is larger, skip the download (0 disables the check) */
          'max-file-size-mb'?: number;
          /** maximum retries for S3 part body download (default 3) */
          'max-retries'?: number;
          /** S3 object key (path) to the file */
          'object-key'?: string;
          /** preallocate the download buffer at startup, so downloads reuse it instead of allocating a per-object buffer; engages only when max-file-size-mb is set (default true) */
          'preallocate-memory'?: boolean;
          /** S3 region */
          'region'?: string;
          /** S3 secret key */
          'secret-key'?: string;
      ...
      /** disable delayed sequencing filter */
      'disable-delayed-sequencing-filter'?: boolean;
      /** enable transaction filtering */
      'enable'?: boolean;
      /** enable address filtering for eth_estimateGas and eth_call */
      'enable-ethcall-filter'?: boolean;
      'event-filter'?: {
        /** path to JSON file containing event filter rules */
        path?: string;
      };
      'filtering-report-rpc-client'?: {
        /** limit size of arguments in log entries (default 2048) */
        'arg-log-limit'?: number;
        /** how long to wait for initial connection */
        'connection-wait'?: string;
    ...
    'tx-indexer'?: {
      /** enables transaction indexer (default true) */
      'enable'?: boolean;
      /** minimum delay between transaction indexing/unindexing batches; the bigger the delay, the more blocks can be included in each batch (default 1s) */
      'min-batch-delay'?: string;
      /** number of threads used to RLP decode blocks during indexing/unindexing of historical transactions (default 14) */
      'threads'?: number;
    ...
    'vmtrace'?: {
      /** (experimental) Tracer configuration in JSON format (default "{}") */
      'json-config'?: string;
      /** (experimental) Name of tracer which should record internal VM operations (costly) */
      'tracer-name'?: string;
    };
  };
  'file-logging'?: {
    ...
    'max-age'?: number;
    /** maximum number of old log files to retain (0 = no limit) (default 40) */
    'max-backups'?: number;
  ...
  'init'?: {
    ...
    /** path to save temp downloaded file */
    ...
    /** directory path for genesis json files - will search for a file named by the chain ID */
    'genesis-json-file-directory'?: string;
    ...
    /** the number of threads to use when pruning (default 14) */
    ...
    /** tests genesis assertion posted on parent chain against the genesis block created on init (default true) */
    'validate-genesis-assertion'?: boolean;
  };
  'ipc'?: {
  ...
  'node'?: {
    'batch-poster'?: {
      /** In AnyTrust mode, the period which AnyTrust nodes are requested to retain the stored batches. (default 360h0m0s) */
      'anytrust-retention-period'?: string;
      ...
      /** DEPRECATED: use compression-levels instead. batch compression level */
      'compression-level'?: number;
      /** JSON array of compression level steps. Format: [{"backlog":<int>,"level":<int>,"recompression-level":<int>},...]. First entry must have backlog:0. Both Level and recomp-level must be 0-11, weakly descending. Example: [{"backlog":0,"level":11,"recompression-level":11},{"backlog":21,"level":6,"recompression-level":11}] (default null) */
      'compression-levels'?: string;
      'dangerous'?: {
      ...
      'error-delay'?: string;
      /** number of batches to post to EthDA before retrying AltDA after a fallback (default 10) */
      'ethda-fallback-batch-count'?: number;
      ...
      /** maximum estimated compressed calldata batch size (default 100000) */
      'max-calldata-batch-size'?: number;
      /** maximum batch posting delay (default 1h0m0s) */
      'max-delay'?: string;
      /** maximum empty batch posting delay, batch poster will only be able to post an empty batch if this time period building a batch has passed; if 0, disable automatic empty batch posting (default 72h0m0s) */
      'max-empty-batch-delay'?: string;
      /** DEPRECATED: use node.batch-poster.max-calldata-batch-size instead */
      'max-size'?: number;
      ...
      'redis-lock'?: {
        /** should node always try grabbing lock in background */
    ...
    'block-metadata-fetcher'?: {
      ...
      'source'?: {
        ...
        /** Errors matching this regular expression are automatically retried (default "websocket: close.*|dial tcp .*|.*i/o timeout|.*connection reset by peer|.*connection refused") */
        'retry-errors'?: string;
        /** per-response timeout (0-disabled) (default 10s) */
        'timeout'?: string;
    ...
    'block-validator'?: {
      ...
      'dangerous'?: {
        ...
        'revalidation'?: {
          /** end revalidation at this block */
          'end-block'?: number;
          /** exit node after revalidation is done */
          'quit-after-revalidation'?: boolean;
          /** start revalidation from this block */
          'start-block'?: number;
        };
      };
      ...
      /** record that many blocks ahead of validation (larger footprint) (default 28) */
      ...
      /** limit on block validations to keep in validation sent state (default 1024) */
      'validation-sent-limit'?: number;
      ...
      /** number of attempts allowed when trying to spawn a validation before erroring out (default 1) */
      'validation-spawning-allowed-attempts'?: number;
      /** number of timeout errors allowed per validation attempt before treating it as a fatal error (separate from allowed-attempts) (default 3) */
      'validation-spawning-allowed-timeouts'?: number;
    };
    'bold'?: {
      /** enable api */
      'api'?: boolean;
      ...
      'dangerous'?: {
        /** assume this message index is valid (if blockhash matches) */
        'assume-valid'?: number;
        /** blockhash of the assumed-valid message */
        'assume-valid-blockhash'?: string;
      };
      'delegated-staking'?: {
    ...
    'consensus-execution-syncer'?: {
      /** Interval in which finality and sync data is pushed from consensus to execution (default 300ms) */
      'sync-interval'?: string;
    };
    'da'?: {
      'anytrust'?: {
        ...
        'enable'?: boolean;
        /** maximum batch size for AnyTrust DA (compressed) (default 1000000) */
        ...
        'request-timeout'?: string;
        'rest-aggregator'?: {
          /** how long to wait for initial connection (default 1s) */
          'connection-wait'?: string;
          /** enable retrieval of sequencer batch data from a list of remote REST endpoints; if other AnyTrust storage types are enabled, this mode is used as a fallback */
          'enable'?: boolean;
          ...
          /** a URL to a list of URLs of REST AnyTrust endpoints that is checked at startup; additive with the url option */
          ...
          'sync-to-storage'?: {
            ...
            /** eagerly sync batch data to this AnyTrust server's storage from the rest endpoints, using L1 as the index of batch data hashes; otherwise only sync lazily */
            'eager'?: boolean;
          ...
          /** list of URLs including 'http://' or 'https://' prefixes and port numbers to REST AnyTrust endpoints; additive with the online-url-list option */
          'urls'?: string[];
        ...
        'rpc-aggregator'?: {
          ...
          'backends'?: string;
          /** enable storage of sequencer batch data from a list of RPC endpoints; this should only be used by the batch poster and not in combination with other AnyTrust storage types */
          'enable'?: boolean;
          'rpc-client'?: {
            'data-stream'?: {
              /** maximum HTTP body size for chunked store requests (default 5242880) */
              'max-store-chunk-body-size'?: number;
              'rpc-methods'?: {
                /** name of the RPC method to finalize a chunked data stream (default "das_commitChunkedStore") */
                'finalize-stream'?: string;
                /** name of the RPC method to start a chunked data stream (default "das_startChunkedStore") */
                'start-stream'?: string;
                /** name of the RPC method to send a chunk of data (default "das_sendChunk") */
                'stream-chunk'?: string;
              };
            };
            /** enable data to be sent to AnyTrust in chunks instead of all at once (default true) */
      ...
      'external-provider'?: {
        'data-stream'?: {
          ...
          'max-store-chunk-body-size'?: number;
          'rpc-methods'?: {
            /** name of the RPC method to finalize a chunked data stream (default "daprovider_commitChunkedStore") */
            'finalize-stream'?: string;
            /** name of the RPC method to start a chunked data stream (default "daprovider_startChunkedStore") */
            'start-stream'?: string;
            /** name of the RPC method to send a chunk of data (default "daprovider_sendChunk") */
            'stream-chunk'?: string;
          };
        };
        /** enable daprovider client */
        'enable'?: boolean;
        'rpc'?: {
          /** limit size of arguments in log entries (default 2048) */
          'arg-log-limit'?: number;
        ...
        /** name of the store rpc method on the daprovider server (used when data streaming is disabled) (default "daprovider_store") */
        'store-rpc-method'?: string;
        /** use data streaming protocol for storing large payloads */
        'use-data-streaming'?: boolean;
        /** implies if the daprovider rpc server supports writer interface */
        'with-writer'?: boolean;
      };
    };
    'dangerous'?: {
      /** DANGEROUS! for chains being retired off AnyTrust: suppresses the AnyTrust writer and halts on any AnyTrust batch when rest-aggregator is disabled */
      'always-fallback-to-parent-chain-da'?: boolean;
    ...
    'delayed-sequencer'?: {
      ...
      'enable'?: boolean;
      /** how often to do a full re-execution when halted on a filtered delayed message (default 30s) */
      'filtered-tx-full-retry-interval'?: string;
    ...
    'feed'?: {
      ...
      output?: {
        ...
        'addr'?: string;
        'backlog'?: {
          /** enable deep copying of L2 messages for memory profiling (debug only) */
          'enable-backlog-deep-copy'?: boolean;
        ...
        /** sign broadcast messages with the batch poster wallet */
        'signed'?: boolean;
    ...
    'maintenance'?: {
      /** how often to check if maintenance should be run (default 1m0s) */
      'check-interval'?: string;
      /** enable maintenance runner */
      'enable'?: boolean;
      'lock'?: {
        /** should node always try grabbing lock in background */
        'background-lock'?: boolean;
    ...
    'message-extraction'?: {
      /** the number of blocks to prefetch relevant logs from. Recommend using max allowed range for eth_getLogs rpc query (default 499) */
      'blocks-to-prefetch'?: number;
      /** enable message extraction service */
      'enable'?: boolean;
      /** frequency of logging message extraction status in terms of number of blocks processed (default 100) */
      'log-extraction-status-frequency-blocks'?: number;
      /** mode to only read latest or safe or finalized L1 blocks. Enabling safe or finalized disables feed input and output. Defaults to latest. Takes string input, valid strings- latest, safe, finalized (default "latest") */
      'read-mode'?: string;
      /** wait time before retring upon a failure (default 500ms) */
      'retry-interval'?: string;
      /** max times the MEL fsm is allowed to be stuck without logging error (default 10) */
      'stall-tolerance'?: number;
    };
    'message-pruner'?: {
    ...
    'resource-mgmt'?: {
      /** Decline RPC calls if free memory excluding the page cache is below this amount */
      'mem-free-limit'?: string;
    };
    'rpc-server'?: {
      /** rpc is authenticated (default true) */
      authenticated?: boolean;
      /** enable consensus node to serve over rpc */
      enable?: boolean;
      /** rpc is public */
      public?: boolean;
    };
    'seq-coordinator'?: {
      /** expiration duration for block metadata keys in Redis (default 240h0m0s) */
      ...
      /** duration to hold the sequencer lockout after acquiring it (default 1m0s) */
      'lockout-duration'?: string;
      /** time to subtract from lockout duration to ensure timely renewal (default 30s) */
      ...
      /** the quorum size needed to qualify a redis GET as valid (default 1) */
      'redis-quorum-size'?: number;
      ...
      /** interval to wait before retrying after an error (default 50ms) */
      'retry-interval'?: string;
      /** if non-zero will add delay after transferring control (default 5s) */
      'safe-shutdown-delay'?: string;
      /** expiration duration for message count keys in Redis (default 240h0m0s) */
      ...
      /** interval between sequencer coordinator update attempts (default 250ms) */
      'update-interval'?: string;
    };
    /** enable sequencer */
    'sequencer'?: boolean;
    ...
    'transaction-streamer'?: {
      ...
      /** when true (default), on a feed-vs-local block hash mismatch the node refuses to process further messages and shuts down gracefully; set to false only if you trust local execution over the feed and want to keep processing (default true) */
      'shutdown-on-blockhash-mismatch'?: boolean;
    ...
    'version-alerter-server'?: {
      /** enable arb_getMinRequiredNitroVersion endpoint that returns minimum required version of the nitro node software */
      'enable'?: boolean;
      /** minimum required version of the nitro node software by date. Second string in the result of querying arb_getMinRequiredNitroVersion endpoint */
      'min-required-nitro-by-date'?: string;
      /** minimum required version of the nitro node software. First string in the result of querying arb_getMinRequiredNitroVersion endpoint */
      'min-required-nitro-by-version'?: string;
      /** deadline to upgrade the nitro node software. Third string in the result of querying arb_getMinRequiredNitroVersion endpoint */
      'upgrade-deadline'?: string;
    };
  };
  'parent-chain'?: {
    'blob-client'?: {
      ...
      'dangerous'?: {
        /** DANGEROUS! Skips verification of KZG proofs for blobs fetched from the beacon node. */
        'skip-blob-proof-verification'?: boolean;
      };
  ...
  'persistent'?: {
    ...
    'pebble'?: {
      'experimental'?: {
        ...
        /** amount of L0 read-amplification necessary to trigger an L0 compaction (default 2) */
        ...
        /** hard limit on the number of queued of MemTables (default 4) */
        ...
        /** number of bytes to write to a write-ahead log (WAL) before calling Sync on it in the background (default 512000) */
      ...
      /** maximum number of concurrent compactions (default 14) */
  ...
  'validation'?: {
    ...
    'arbitrator'?: {
      ...
      'redis-validation-server-config'?: {
        ...
        'consumer-config'?: {
          /** After a message spends this amount of time in PEL (Pending Entries List i.e claimed by another consumer but not Acknowledged) it will be allowed to be autoclaimed by other consumers. This option should be set to the same value for all consumers and producers. (default 5m0s) */
          'idletime-to-autoclaim'?: string;
          /** number of message retries after which this consumer will set an error response and Acknowledge the message (-1 = no limit) (default -1) */
          'max-retry-count'?: number;
          /** timeout for response entry (default 1h0m0s) */
          'response-entry-timeout'?: string;
          /** enables autoclaim for this consumer, if set to false this consumer will not check messages from PEL (Pending Entries List) (default true) */
          'retry'?: boolean;
        };
    ...
    'jit'?: {
      ...
      'cranelift'?: boolean;
      /** path to jit executable, if empty, attempts to find jit executable relative to nitro binary or in PATH */
      'jit-path'?: string;
    ...
    'use-jit'?: boolean;
    'wasm'?: {
      /** list of WASM module roots or machine base paths to match against on-chain WasmModuleRoot */
      ...
      /** path to machine folders, each containing wasm files (machine.v2.wavm.br, replay.wasm) */
      'root-path'?: string;
    };
  };
  'version-alerter'?: {
    ...
    /** enable querying arb_getMinRequiredNitroVersion endpoint in regular intervals and firing alerts if the node software is below the required version */
    'enable'?: boolean;
    /** how often the nitro version alerter should ping arb_getMinRequiredNitroVersion for data (default 5m0s) */
    'ping-interval'?: string;
    /** represents grace period up until the upgrade deadline received from arb_getMinRequiredNitroVersion, determines escalation of messages regarding node software upgrade */
    'upgrade-grace-period'?: string;
  };
  'ws'?: {
